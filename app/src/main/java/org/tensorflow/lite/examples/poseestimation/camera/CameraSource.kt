...
class CameraSource(
    ...
    private fun ensureImageReaderThread() {
    ...
    suspend fun initCamera() {
        ...
        imageReader?.setOnImageAvailableListener({ reader ->
            ...
            if (image != null) {
                ...
                val baseRotation = 90.0f
                val extraRotation = if (lensFacing == CameraCharacteristics.LENS_FACING_FRONT) 180.0f else 0.0f
                rotateMatrix.postRotate(baseRotation + extraRotation)
    ...
    fun prepareCamera(lensFacing: Int = CameraCharacteristics.LENS_FACING_BACK) {
        this.lensFacing = lensFacing
        var fallbackId: String? = null
        for (candidateId in cameraManager.cameraIdList) {
            val characteristics = cameraManager.getCameraCharacteristics(candidateId)
            val cameraDirection = characteristics.get(CameraCharacteristics.LENS_FACING)
            if (fallbackId == null) {
                fallbackId = candidateId
            }
            if (cameraDirection != null && cameraDirection == lensFacing) {
                this.cameraId = candidateId
                return
            }
        }
        this.cameraId = fallbackId.orEmpty()
    }
